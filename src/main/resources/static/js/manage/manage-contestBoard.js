...
var manageContestBoardPage = {
    ...
    init: function (accountId, pageNum, pageSize, totalPageNum, totalPageSize, contests) {
        ...
        manageContestBoardPage.data.accountId = accountId;
        //分页初始化
    ...
    checkAddContestData: function (contestTitle, startTime, endTime) {
        ...
        if (endTime == null || endTime == ''
            || endTime.replace(/(^s*)|(s*$)/g, "").length == 0) {
            $('#addModalErrorMessage').html('<i class="close icon"></i><div class="header">错误提示</div>\n' +
                '                <p>'+'结束时间不能为空'+'</p>');
            $('#addModalErrorMessage').removeClass('hidden');
            return false;
    ...
    addContestAction: function () {
        ...
        var teacherId = manageContestBoardPage.data.accountId;
